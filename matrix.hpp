...
namespace sjtu //create a namespace
{
	...
	public:
		...
		Matrix(const Matrix<U> &o)//transform class
		{
			...
			for (i = 0; i < o.rowLength(); i++){
				for (j = 0; j < o.columnLength(); j++){
					tmp = (T)o(i, j);
					u1.push_back(tmp);
				}
				u0.push_back(u1);
	...
	public:
		...
		{
			int i, j;
			if (o.rowLength() != rowlen || o.columnLength() != columnlen) throw std::invalid_argument("");
			for (i = 0; i < rowlen; i++){
				for (j = 0; j < columnlen; j++){
					v[i][j] = (T)(o(i, j) + v[i][j]);
				}
	...
	public: // random-access iterator
		...
		public:
			...
			iterator(Matrix<T> * ope, std::pair<size_t , size_t > l)
            {
			    op = &(*ope);
			    pos = l;
            }
		...
		public:
			...
			{
				return (*op)(pos.first, pos.second);
			}

			pointer operator->() const
			{
				T * ans;
				ans = &((*op)(pos.first, pos.second) );
				return ans;
			}
		...
		{
			iterator a;
			a.op = &(*this); //maybe there are some problems...
			a.pos = {0, 0};
			return a;
		}
		...
		std::pair<iterator, iterator> subMatrix(std::pair<size_t, size_t> l, std::pair<size_t, size_t> r)
		{
		    ...
		    std::vector<T> u1;
		    std::vector<std::vector<T>> u0;
		    for (i = l.first; i <= r.first ; i++){
		        for (j = l.second; j <= r.second; j++){
		            u1.push_back(v[i][j]);
		        }
		        u0.push_back(u1);
		        u1.clear();
		    }
		    static Matrix<T> subM = {r.first-l.first+1, r.second-l.second+1 ,u0};
            iterator a = subM.begin();
            iterator b = subM.end();
            std::pair<iterator, iterator> ans = {a, b};
            return ans;
        }
	};
...
namespace sjtu
{
	...
	auto operator*(const Matrix<U> &a, const Matrix<V> &b)->Matrix<decltype(U() * V())>
	{
		...
		if (a.columnLength() != b.rowLength()) throw std::invalid_argument("");
