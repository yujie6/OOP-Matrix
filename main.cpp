...
std::pair<bool, std::string> RE(const std::string &name) //put in the name such as ("tran")
{
...
std::pair<bool, std::string> testConst() //this is the test function
{
    ...
    const Matrix<int> std_col0 = {{ 1 },
                                  { 3 }};
    if (a.tran() != std_tran) //std_tran is the right answer, while a.tran() is mine
        WA("tran");
    if (a.row(0) != std_row0) //return the 0th row
        WA("row0");
    ...
    if (a.columnLength() != 2)
        WA("columnLength");
    if (a + a != a * 2 || a * 2 - a != a) //test overload function
        WA("operator wrong");
    return { true, "Congratulation!" }; //why this? no if??
};

void ceTest() //operation between 2 class
{
    Matrix<int> a;
    Matrix<double> b;
